...
import { CanvasEditorInner, type Peer } from './CanvasEditorInner'
...
export function CanvasEditor() {
  ...
  // Load meta whenever the canvas changes.
  useEffect(() => {
    if (!docId) return
    setError(null)
    ...
    fetchDoc(docId).catch((e) => {
      ...
      if (e instanceof ApiRequestError && (e.status === 404 || e.status === 403)) {
        setError("This canvas doesn't exist or you don't have access to it.")
  ...
  // Access-lost detection. Once the canvas has loaded, it vanishing from the
  // store means our role was revoked (my_role -> 'none') or it was permanently
  ...
  if (error) {
    return (
      <div className="flex min-w-0 flex-1 flex-col items-center justify-center gap-2 bg-[var(--color-ink)] text-center">
        ...
        <Button variant="outline" size="sm" className="mt-2" onClick={() => navigate('/canvas')}>
          Back to canvases
        </Button>
      </div>
    )
    ...
    return (
      <div className="flex min-w-0 flex-1 flex-col items-center justify-center gap-2 bg-[var(--color-ink)] text-center">
        ...
        <p className="max-w-sm text-sm text-[var(--color-text-dim)]">
          You no longer have access to this canvas, or it was deleted.
        </p>
        <Button variant="outline" size="sm" className="mt-2" onClick={() => navigate('/canvas')}>
          Back to canvases
        </Button>
  ...
  // of `canEdit`, which is false on a trashed canvas.
  ...
  async function onTrash() {
    ...
    try {
      ...
      navigate(`/canvas/c/${doc!.channel_id}`)
    } catch (e) {
      if (e instanceof Error) toastError(e.message)
    }
  }
  ...
  return (
    <div className="flex min-w-0 flex-1 flex-col bg-[var(--color-ink)]">
      ...
      <header className="flex h-14 shrink-0 items-center gap-2 border-b border-[var(--color-border)] px-3 sm:gap-3 sm:px-5">
        ...
        <div className="min-w-0 flex-1 truncate text-sm"><button onClick={() => navigate('/canvas')} className="text-[var(--color-text-faint)] hover:text-[var(--color-text)]">‹ Canvas</button><span className="mx-1.5 text-[var(--color-text-faint)]">/</span><span className="text-[var(--color-text-dim)]">{doc.title || 'Untitled'}</span></div>
      ...
      <div className="shrink-0 px-4 pt-4 pb-3 sm:px-6">
        {trashed && (
          ...
          >
            This canvas is in the trash.
          </Banner>
        )}
        {!trashed && isViewer && (
          <Banner tone="neutral" className="mb-3">
            You have read-only access to this canvas.
          </Banner>
        )}
