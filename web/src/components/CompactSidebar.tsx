...
type Mode = 'chat' | 'docs' | 'canvas' | 'meetings' | 'calendar'
...
export function CompactSidebar({ mode }: { mode: Mode }) {
  ...
  return (
    <aside className="flex h-full w-[4.5rem] shrink-0 flex-col border-r border-[var(--color-border)] bg-[var(--color-panel)]">
      ...
      <nav
        aria-label={`${mode === 'chat' ? 'Chat' : mode === 'docs' ? 'Docs' : mode === 'canvas' ? 'Canvas' : mode === 'calendar' ? 'Calendar' : 'Meetings'} sidebar`}
      ...
      >
        ...
        {mode !== 'chat' && (
          <CompactLink
            ...
            label={`${mode === 'docs' ? 'Docs' : mode === 'canvas' ? 'Canvas' : mode === 'calendar' ? 'Calendar' : 'Meetings'} home`}
            active={
              location.pathname ===
              (mode === 'docs'
                ? '/docs'
