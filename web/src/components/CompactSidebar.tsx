...
type Mode = 'chat' | 'docs' | 'canvas' | 'meetings'
...
export function CompactSidebar({ mode }: { mode: Mode }) {
  ...
  return (
    <aside className="flex h-full w-[4.5rem] shrink-0 flex-col border-r border-[var(--color-border)] bg-[var(--color-panel)]">
      ...
      <nav
        aria-label={`${mode === 'chat' ? 'Chat' : mode === 'docs' ? 'Docs' : mode === 'canvas' ? 'Canvas' : 'Meetings'} sidebar`}
      ...
      >
        ...
        {mode !== 'chat' && (
          <CompactLink
            to={mode === 'docs' ? '/docs' : mode === 'canvas' ? '/canvas' : '/meetings'}
            label={`${mode === 'docs' ? 'Docs' : mode === 'canvas' ? 'Canvas' : 'Meetings'} home`}
            active={
              location.pathname === (mode === 'docs' ? '/docs' : mode === 'canvas' ? '/canvas' : '/meetings')
            }
          >
            <HomeIcon />
          </CompactLink>
        ...
        {mode !== 'meetings' && myChannels.map((channel) => (
          <CompactChannelLink
            key={channel.id}
            channel={channel}
            mode={mode}
