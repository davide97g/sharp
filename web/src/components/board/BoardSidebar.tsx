...
export function BoardSidebar() {
  ...
  const activeBoardId = location.pathname.match(/^\/b\/([^/]+)/)?.[1] ?? null
  const activeChannelId = useStore((s) =>
    activeBoardId ? s.docMeta[activeBoardId]?.channel_id ?? null : null,
  )
  ...
  return (
    <aside className="flex w-64 shrink-0 flex-col border-r border-[var(--color-border)] bg-[var(--color-panel)]">
      <div className="flex h-14 items-center gap-2 border-b border-[var(--color-border)] px-4">
        ...
        <span className="text-base font-bold tracking-tight">Board</span>
      </div>
        ...
        <div className="mt-1 space-y-0.5">
          {myChannels.length === 0 && (
            <div className="px-2 py-1.5 text-sm text-[var(--color-text-faint)]">
              Join a channel to start planning.
            </div>
          )}
          {myChannels.map((c) => (
            <ChannelBoardGroup
...
function ChannelBoardGroup({
  ...
  // Boards share the docs buckets — filter to kind === 'board'.
  const boards = useMemo(() => docs?.filter((d) => d.kind === 'board'), [docs])

  // Auto-expand if the active board belongs to this channel.
  const hasActive = activeChannel || !!boards?.some((d) => d.id === activeBoardId)
  const expanded = open || hasActive

  // Load the active board's channel so the sidebar can highlight it.
  useEffect(() => {
  ...
  async function newBoard(e: React.MouseEvent) {
    e.stopPropagation()
    if (creating) return
    setCreating(true)
    try {
      const board = await createBoard(channel.id)
      setOpen(true)
      navigate(`/b/${board.id}`)
    } catch (err) {
    ...
    <div>
      ...
      {expanded && (
        <div className="mb-1 ml-4 space-y-0.5 border-l border-[var(--color-border)] pl-1.5">
          ...
          {boards?.map((d) => (
            <BoardRow key={d.id} board={d} active={d.id === activeBoardId} />
          ))}
          <NavLink
            to={`/board/c/${channel.id}`}
...
function BoardRow({ board, active }: { board: Doc; active: boolean }) {
  return (
    <NavLink
      to={`/b/${board.id}`}
    ...
    >
      <span className="w-4 shrink-0 text-center text-xs">{board.icon || '🗂️'}</span>
      <span className="min-w-0 flex-1 truncate">{board.title || 'Untitled'}</span>
    </NavLink>
  )
}
