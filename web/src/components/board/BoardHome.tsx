...
export function BoardHome() {
  ...
  // (loadChannelDocs fetches docs, canvases and boards in one request.)
  useEffect(() => {
  ...
  return (
    <div className="flex min-w-0 flex-1 flex-col bg-[var(--color-ink)]">
      <header className="flex h-14 items-center gap-2 border-b border-[var(--color-border)] px-5">
        ...
        <span className="text-sm text-[var(--color-text-dim)]">Your boards</span>
      </header>
              ...
              {recent.length === 0 ? (
                <div className="rounded-xl border border-dashed border-[var(--color-border)] px-6 py-12 text-center">
                  ...
                  <p className="text-sm text-[var(--color-text-dim)]">
                    No boards yet. Pick a channel in the sidebar and create one.
                  </p>
                </div>
              ) : (
                  ...
                  {recent.map((d) => (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <span className="text-xl">{d.icon || '🗂️'}</span>
...
function MentionsInbox() {
  ...
  // Board mode shows board mentions only; doc/canvas mentions live in their modes.
  const mentions = allMentions.filter((m) => m.doc.kind === 'board')
        ...
        <div className="space-y-1.5">
          {mentions.map((m) => {
            ...
            return (
              ...
              >
                <span className="text-lg">{m.doc.icon || '🗂️'}</span>
