...
export function ChannelBoards() {
  ...
  // Store buckets hold docs, canvases and boards; keep only boards here.
  const boards = useMemo(() => docs?.filter((d) => d.kind === 'board'), [docs])
  const trashedBoards = useMemo(() => trash?.filter((d) => d.kind === 'board'), [trash])

  async function newBoard() {
    if (!channelId) return
    try {
      const doc = await createBoard(channelId)
      navigate(`/b/${doc.id}`)
    } catch (err) {
  ...
  return (
    <div className="flex min-w-0 flex-1 flex-col bg-[var(--color-ink)]">
      ...
      <ChannelTabs channelId={channelId} active="board" />
...
function TrashRow({ doc }: { doc: Doc }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg border border-[var(--color-border)] bg-[var(--color-panel)] px-4 py-2.5">
      <span className="text-lg opacity-60">{doc.icon || '🗂️'}</span>
