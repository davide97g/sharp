...
export function QuickSwitcher() {
  ...
  const items = useMemo<Item[]>(() => {
    ...
    const docItems: Item[] = Object.values(docsByChannel)
      ...
      .map((d) => ({
        ...
        sub: `${d.kind === 'canvas' ? 'Canvas' : d.kind === 'board' ? 'Board' : 'Doc'} · #${chanName[d.channel_id] ?? ''}`,
        icon: d.icon || (d.kind === 'canvas' ? '🎨' : d.kind === 'board' ? '🗂️' : '📄'),
        docKind: d.kind,
  ...
  const filtered = useMemo(() => {
    ...
    const pool = prefix
      ? [...items, ...commandItems].filter((it) => it.kind === prefix)
      : items
    const all = pool
  ...
  async function choose(it: Item) {
    ...
    if (it.kind === 'channel') {
      ...
      const prefix = it.docKind === 'canvas' ? '/x' : it.docKind === 'board' ? '/b' : '/d'
      navigate(`${prefix}/${it.id}`)
    } else {
      try {
      ...
      >
        ...
        <div className="border-t border-[var(--color-border)] px-4 py-2 text-2xs text-[var(--color-text-faint)]">
          ...
          <span className="ml-2 opacity-70">
            ...
            <b className="font-semibold">/</b> docs
          </span>
        </div>
      </div>
    </div>
