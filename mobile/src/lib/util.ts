// keep in sync with web/src/lib/util.ts
import type { Channel } from './types'

const resourceToken = /\[\[(doc|canvas):([0-9a-f-]{36})\|([^\]]*)\]\]/g

export function stripResourceTokens(text: string): string {
  return text.replace(resourceToken, (_all, kind, _id, title) => `${kind === 'doc' ? '📄' : '🎨'} ${title || 'Untitled'}`)
}
