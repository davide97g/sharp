...
pub struct Config {
    ...
    /// Outbound transactional email (password reset). `None` unless a backend is
    /// configured — the reset flow is then inert. Resend wins over SMTP when both
    /// are present.
...
#[derive(Clone)]
pub enum MailConfig {
    /// Resend's HTTP API — no outbound SMTP ports needed.
    Resend(ResendConfig),
    /// Any SMTP relay (lettre).
    Smtp(SmtpConfig),
}
...
pub struct ResendConfig {
    pub api_key: String,
    /// From address (e.g. `Sharp <no-reply@example.com>`). The domain must be
    /// verified in Resend, or the API rejects the send.
    pub from: String,
    /// API base, overridable for testing. Defaults to `https://api.resend.com`.
    pub base_url: String,
}
...
impl Config {
    pub fn from_env() -> Result<Self, String> {
        ...
        // Email needs a From address whichever backend sends it. EMAIL_FROM is the
        // backend-neutral name; SMTP_FROM stays accepted so existing deploys keep
        // working unchanged.
        let mail_from = env_opt("EMAIL_FROM").or_else(|| env_opt("SMTP_FROM"));

        // Resend takes precedence over SMTP when both are configured: it needs no
        // outbound SMTP ports, which are commonly blocked by hosting providers.
        let mail = match (env_opt("RESEND_API_KEY"), &mail_from) {
            (Some(api_key), Some(from)) => Some(MailConfig::Resend(ResendConfig {
                api_key,
                from: from.clone(),
                base_url: env_opt("RESEND_BASE_URL")
            ...
            _ => match (env_opt("SMTP_HOST"), &mail_from) {
                (Some(host), Some(from)) => {
                    ...
                    Some(MailConfig::Smtp(SmtpConfig {
                        host,
                        port,
