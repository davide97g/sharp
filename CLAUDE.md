...
- **Voice/video calls (Phase 4)**: ephemeral **P2P-mesh WebRTC** audio rooms — one per channel/DM, plus optional **webcam video** and **screen sharing** (with system/tab audio). Server does signaling + media-state coordination only (no SFU, no media relay, no DB tables — room registry lives in server memory). Caps are server-authoritative: **8 audio / 4 cameras / 1 screen share** per room. Client engine is `web/src/lib/voice.ts` (`VoiceClient` — perfect-negotiation, per-track bitrate caps, RMS speaking detection); UI is `web/src/components/voice/` (`VideoStage` adaptive grid + draggable/resizable + in-page fullscreen stage, `CallChatRail`, `VoiceMiniWidget`). **Picture-in-picture** (`web/src/lib/pip.ts`): Meet-style Document PiP with element-PiP fallback so a call floats while you browse other channels. All state rides the existing main WS (`voice.*` events — see Phase 4 in ARCHITECTURE.md); `server/src/ws/voice.rs` owns the room registry. **ICE**: `GET /voice/config` returns STUN/TURN from env (`STUN_URLS`, `TURN_URL`/`TURN_USERNAME`/`TURN_PASSWORD`). **Guest call links** (`server/src/routes/call_links.rs`): a member mints a public link (`voice_link_token` on the channel, regenerate to revoke); anyone joins the room as a name-only guest via a limited guest JWT — no account, no channel membership (`web/src/components/GuestCall.tsx`).
